...
//-----------------------------------------------------------------------------
	...
	int POINT_SOCKET, SESSION_SOCKET, SLIDER_SOCKET, USER_TRACKING_SOCKET/*, DEPTH_MAP_SOCKET*/;
#else
    #include "socket.h"
	SOCKET POINT_SOCKET;
	SOCKET SESSION_SOCKET;
	SOCKET SLIDER_SOCKET;
	SOCKET USER_TRACKING_SOCKET;
	//SOCKET DEPTH_MAP_SOCKET;
#endif
...
XnBool _printGestureOnly = false;
XnBool _printCircleOnly = false;
XnBool _printSliderOnly = false;
XnBool _printTrackPadOnly = false;
XnBool _printSinglePointOnly = false;
XnBool _printUserTrackingOnly = false;
...
#define PUSH "event:push"
#define SWIPE_UP "event:swipe_up"
#define SWIPE_DOWN "event:swipe_down"
#define SWIPE_LEFT "event:swipe_left"
#define SWIPE_RIGHT "event:swipe_right"
#define STEADY "event:steady"
...
void XN_CALLBACK_TYPE CircleCB(XnFloat fTimes, XnBool bConfident, const XnVCircle* pCircle, void* pUserCxt)
{
	...
	if(_printCircleOnly) printf("Circle Angle:%6.2f\n", fAngle);
...
{
	if(_printCircleOnly) printf("No Circle\n");
...
{
	if(_printCircleOnly) printf("Circle Primary Create\n");
...
{
	if(_printCircleOnly) printf("Circle Primary Destroy\n");
...
{
	if(_printGestureOnly) printf("Swipe Up\n");
	if(_useSockets) sendToSocket(SESSION_SOCKET, SWIPE_UP);
}

void XN_CALLBACK_TYPE Swipe_SwipeDown(XnFloat fVelocity, XnFloat fAngle, void* cxt)
{
	if(_printGestureOnly) printf("Swipe Down\n");
	if(_useSockets) sendToSocket(SESSION_SOCKET, SWIPE_DOWN);
}

void XN_CALLBACK_TYPE Swipe_SwipeLeft(XnFloat fVelocity, XnFloat fAngle, void* cxt)
{
	if(_printGestureOnly) printf("Swipe Left\n");
	if(_useSockets) sendToSocket(SESSION_SOCKET, SWIPE_LEFT);
}

void XN_CALLBACK_TYPE Swipe_SwipeRight(XnFloat fVelocity, XnFloat fAngle, void* cxt)
{
	if(_printGestureOnly) printf("Swipe Right\n");
	if(_useSockets) sendToSocket(SESSION_SOCKET, SWIPE_RIGHT);
}

void XN_CALLBACK_TYPE OnWave(void* cxt)
{
	if(_printGestureOnly) printf("Wave Occured\n");
	if(_useSockets) sendToSocket(SESSION_SOCKET, WAVE);
}

void XN_CALLBACK_TYPE onPush(XnFloat fVelocity, XnFloat fAngle, void* cxt)
{
	if(_printGestureOnly) printf("Push Occured\n");
	if(_useSockets) sendToSocket(SESSION_SOCKET, PUSH);
}

void XN_CALLBACK_TYPE Steady_OnSteady(XnFloat fVelocity, void* cxt)
{
	if(_printGestureOnly) printf("Steady Occured\n");
	if(_useSockets) sendToSocket(SESSION_SOCKET, STEADY);
}
...
{
	if(_printSliderOnly) printf("Slider - LeftRight - Value:%f\n", fValue);
	char cValue[50];
...
{
	if(_printSliderOnly) printf("Slider - LeftRight - Activated\n");
...
{
	if(_printSliderOnly) printf("Slider - LeftRight - Deactivated\n");
...
{
	if(_printSliderOnly) printf("Slider - LeftRight - Created\n");
...
{
	if(_printSliderOnly) printf("Slider - LeftRight - Destroyed\n");
...
{
	if(_printSliderOnly) printf("Slider - UpDown - Value:%f\n", fValue);
	char cValue[50];
...
{
	if(_printSliderOnly) printf("Slider - UpDown - Activated\n");
...
{
	if(_printSliderOnly) printf("Slider - UpDown - Deactivated\n");
...
{
	if(_printSliderOnly) printf("Slider - UpDown - Created\n");
...
{
	if(_printSliderOnly) printf("Slider - UpDown - Destroyed\n");
...
{
	if(_printSliderOnly) printf("Slider - InOut - Value:%f\n", fValue);
	char cValue[50];
...
{
	if(_printSliderOnly) printf("Slider - InOut - Created\n");
...
{
	if(_printSliderOnly) printf("Slider - InOut - Destroyed\n");
...
{
	//if(_printTrackPadOnly) printf("TrackPad Value: %f, %f\n", fXValue, fYValue);
}

void XN_CALLBACK_TYPE TrackPad_ItemHover(XnInt32 nXItem, XnInt32 nYItem, void* cxt)
{
  	if(_printTrackPadOnly) printf("TrackPad Hover: %d,%d\n", nXItem, nYItem);
	char cValue[50];
...
{
	if(_printTrackPadOnly) printf("TrackPad Select: %d,%d (%s)\n", nXItem, nYItem, XnVDirectionAsString(eDir));
	char cValue[50];
...
{
  	if(_printTrackPadOnly) printf("TrackPad Destroyed\n");
...
{
	if(_printUserTrackingOnly) printf("New User: %d\n", nId);
	if(_needPose)
	{
...
{
	if(_printUserTrackingOnly) printf("Lost user: %d\n", nId);
...
{
	if(_printUserTrackingOnly) printf("Pose %s detected for user: %d\n", strPose, nId);
...
{
	if(_printUserTrackingOnly) printf("Calibration started for user: %d\n", nId);
...
void XN_CALLBACK_TYPE UserCalibration_CalibrationEnd(SkeletonCapability& capability, XnUserID nId, XnBool bSuccess, void* pCookie)
{
	if (bSuccess)
	{
		if(_printUserTrackingOnly) printf("Calibration complete, start tracking user: %d\n", nId);
	...
	{
		if(_printUserTrackingOnly) printf("Calibration failed for user: %d\n", nId);
		if (_needPose)
		{
			_userGenerator.GetPoseDetectionCap().StartPoseDetection(_strPose, nId);
		}
...
{	
	...
	if(_featureTrackPad)
	{
		...
		_trackPad->RegisterValueChange(NULL, &TrackPad_ValueChange);
