...
{
	printf("AS3OpenNI :: Waiting for client\n");
	dataClientSocket = accept(dataSocket, NULL, NULL);
	
	#if (XN_PLATFORM == XN_PLATFORM_WIN32)
		if (dataClientSocket == INVALID_SOCKET && !_quit)
		{
			printf("AS3OpenNI :: Error on accept() for data, exit data thread. %d\n", WSAGetLastError());
			closesocket(dataSocket);
			WSACleanup();
		}
	#else
		if (0 > dataClientSocket && !_quit) 
		{
			printf("AS3OpenNI :: Error on accept() for data, exit data thread. %s\n", strerror(errno));
			close(dataSocket);
		}
	#endif

	printf("AS3OpenNI :: Got data client\n");
	networkCallback();
}
...
int network::init(callback cb) 
{
	...
	int network::initServer(addrinfo si_type, PCSTR conf_port, PCSTR label)
	{
	    ...
	    for(serverObj=serverResult; serverObj != NULL; serverObj=serverObj->ai_next) 
	    {
	        ...
	        {
	            printf("AS3OpenNI :: Error at session socket(): %ld\n", WSAGetLastError());
				return 1;
	        }
			...
			{
				printf("AS3OpenNI :: %s: bind failed: %d\n", label, WSAGetLastError());
			...
			{
				printf("AS3OpenNI :: %s: listen failed [%ld]\n", label, WSAGetLastError() );
				closesocket(dataSocket);
				WSACleanup();
				return 1;
			}
	    ...
	    if(dataSocket == INVALID_SOCKET) 
	    {
	        printf("AS3OpenNI :: Unable to connect to session server!\n");
			return 1;
	    }
	...
	int network::initServer(addrinfo si_type, const char *conf_port, const char *label)
	{
		...
		if (iResult < 0) 
		{
			printf("AS3OpenNI :: %s: getaddrinfo failed: %d\n", label, iResult);
			return 1;
		}
		...
		for(serverObj=serverResult; serverObj != NULL; serverObj=serverObj->ai_next) 
	    {
	    	...
	    	if (dataSocket < 0)
			{
	    		printf("AS3OpenNI :: %s: socket failed [%s]\n", label, strerror(errno));
			...
			if (iResult < 0) 
			{
				printf("AS3OpenNI :: %s: bind failed: %s\n", label, strerror(errno));
				freeaddrinfo(serverResult);
				close(dataSocket);
				return 1;
			}
			...
			{
				printf("AS3OpenNI :: %s: listen failed [%s]\n", label, strerror(errno));
				close(dataSocket);
				return 1;
			}
